...
async function createWorkbook(styles:StyleData[],date=new Date(),internalReview=false):Promise<ExcelJS.Workbook>{
  ...
  for(const style of styles){
    ...
    if(internalReview){
      const erp=style.erpMaterialCost??0,ratio=style.finalFob?materialTotal/style.finalFob:'',difference=materialTotal-erp,differenceRate=erp?difference/erp:'';
      const ratioRow=ws.addRow(['INTERNAL REVIEW','','','','','','',{formula:`IFERROR(H${materialTotalRow}/H${fobRow},"")`,result:ratio},'CBD 재료비 / FOB']);
      const erpRow=ws.addRow(['','','','','','','',{formula:`SUMIFS('ERP MATERIAL CHECK'!$M:$M,'ERP MATERIAL CHECK'!$A:$A,"${String(style.id).replace(/"/g,'""')}")`,result:erp},'사전원가 재료비']);
