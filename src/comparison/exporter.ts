...
const pairedDifferenceFormula=(referenceCell:string,comparisonCell:string)=>`IF(AND(ISNUMBER(${referenceCell}),ISNUMBER(${comparisonCell})),${comparisonCell}-${referenceCell},\"\")`;
...
export function buildComparisonWorkbook(state:ComparisonState){
 ...
 for(const styleMatch of state.styleMatches.filter(match=>!match.excluded)){
  ...
  summary.addRow([state.referenceSeason,reference?.styleName||'—',state.currentSeason,comparison?.styleName||'—',styleMatch.method,excelStatus(styleMatch.status),reference?.summary.totalMaterialCost??'',comparison?.summary.totalMaterialCost??'',reference?.summary.fob??'',comparison?.summary.fob??'']);
  ...
  for(const cluster of ordered){
   ...
   sheet.getCell(row,5).value=value(total('cost'));sheet.getCell(row,6).value=value(comparisonRow?.cost);sheet.getCell(row,7).value={formula:pairedDifferenceFormula(`E${row}`,`F${row}`)};
   sheet.getCell(row,8).value=value(total('usage'));sheet.getCell(row,9).value=value(comparisonRow?.usage);sheet.getCell(row,10).value={formula:pairedDifferenceFormula(`H${row}`,`I${row}`)};
