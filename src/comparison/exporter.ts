...
export function buildComparisonWorkbook(state:ComparisonState,options?:Pick<ComparisonExportOptions,'stylePairIds'>){
 ...
 for(const styleMatch of state.styleMatches.filter(match=>!match.excluded&&(!selectedIds||selectedIds.has(match.id)))){
  const reference=style(state,styleMatch.referenceId),comparison=style(state,styleMatch.currentId),clusters=state.materialMatches.find(set=>set.styleMatchId===styleMatch.id)?.clusters||[],summaryRow=summary.addRow([state.referenceSeason,reference?.styleName||'—',state.currentSeason,comparison?.styleName||'—',styleMatch.method,excelStatus(styleMatch.status),null,null,reference?.summary.fob??null,comparison?.summary.fob??null]),sheet=workbook.addWorksheet(safeName(comparison?.styleName||reference?.styleName||'Unmatched',used));
  ...
  sheet.getCell('A5').value='FOB 및 그룹별 재료비 비교';sheet.getCell('A5').font={bold:true,size:13};
  ...
  const fobRow=7,fobReference=reference?.summary.fob,fobComparison=comparison?.summary.fob;sheet.getCell(fobRow,1).value=fobReference===undefined||fobComparison===undefined?'FOB (검토 필요)':'FOB';sheet.getCell(fobRow,2).value=formula(sheetReference('STYLE MATCH',`I${summaryRow.number}`),fobReference??'');sheet.getCell(fobRow,3).value=formula(sheetReference('STYLE MATCH',`J${summaryRow.number}`),fobComparison??'');sheet.getCell(fobRow,4).value=fobReference!==undefined&&fobComparison!==undefined?formula(`C${fobRow}-B${fobRow}`,fobComparison-fobReference):null;sheet.getCell(fobRow,5).value=fobReference!==undefined&&fobComparison!==undefined?formula(`IF(B${fobRow}=0,"",D${fobRow}/B${fobRow})`,fobReference===0?'':(fobComparison-fobReference)/fobReference):null;sheet.getRow(fobRow).font={bold:true};sheet.getRow(fobRow).fill=fill('DBEAFE');
  let groupRow=8;
