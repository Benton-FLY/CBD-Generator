...
export function buildComparisonWorkbook(state:ComparisonState,options?:Pick<ComparisonExportOptions,'stylePairIds'>){
 ...
 for(const styleMatch of state.styleMatches.filter(match=>!match.excluded&&(!selectedIds||selectedIds.has(match.id)))){
  ...
  for(const group of groups){
   ...
   if(useReferenceOriginal||useComparisonOriginal){sourceRow=row++;sheet.getCell(sourceRow,1).value=`${group} 원본 CBD 소계 (참고)`;sheet.getCell(sourceRow,11).value=useReferenceOriginal?referenceOriginal!:null;sheet.getCell(sourceRow,12).value=useComparisonOriginal?comparisonOriginal!:null;sheet.getCell(sourceRow,13).value=null;sheet.getCell(sourceRow,15).value='원본 CBD 소계: 참고 전용, 비교 합산 제외';sheet.getRow(sourceRow).fill=fill('FEF3C7');}
   const subtotalRow=row++,detailEnd=sourceRow?sourceRow-1:subtotalRow-1,referenceValue=referenceDetail,comparisonValue=comparisonDetail;
   sheet.getCell(subtotalRow,1).value=`${group} 최종 비교 소계`;sheet.getCell(subtotalRow,11).value=formula(detailEnd>=start?`SUM(K${start}:K${detailEnd})`:`0`,referenceValue);sheet.getCell(subtotalRow,12).value=formula(detailEnd>=start?`SUM(L${start}:L${detailEnd})`:`0`,comparisonValue);sheet.getCell(subtotalRow,13).value=formula(`L${subtotalRow}-K${subtotalRow}`,comparisonValue-referenceValue);sheet.getRow(subtotalRow).font={bold:true};sheet.getRow(subtotalRow).fill=fill('DBEAFE');subtotals.set(group,{row:subtotalRow,reference:referenceValue,comparison:comparisonValue});
  }
