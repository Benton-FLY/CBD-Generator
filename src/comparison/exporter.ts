...
export function buildComparisonWorkbook(state:ComparisonState,options?:Pick<ComparisonExportOptions,'stylePairIds'>){
 ...
 for(const styleMatch of state.styleMatches.filter(match=>!match.excluded&&(!selectedIds||selectedIds.has(match.id)))){
  ...
  [20,42,42,9,12,12,12,12,12,12,14,14,14,18,30].forEach((width,index)=>sheet.getColumn(index+1).width=width);
  sheet.eachRow(item=>item.alignment={vertical:'top'});for(let index=6;index<=totalRow;index++){sheet.getCell(index,1).alignment={horizontal:'left',vertical:'middle'};for(let column=2;column<=5;column++)sheet.getCell(index,column).alignment={horizontal:'right',vertical:'middle'}}
  for(let index=14;index<=15;index++){sheet.getCell(index,1).alignment={horizontal:'left',vertical:'middle'};for(let column=2;column<=15;column++)sheet.getCell(index,column).alignment={horizontal:column===4||column===14?'center':column>=5&&column<=13?'right':'left',vertical:'middle',wrapText:column===2||column===3||column===14||column===15}}
  for(let index=16;index<row;index++){sheet.getCell(index,1).alignment={horizontal:'left',vertical:'top'};for(const column of [2,3,15])sheet.getCell(index,column).alignment={horizontal:'left',vertical:'top',wrapText:true};sheet.getCell(index,4).alignment={horizontal:'center',vertical:'top'};for(let column=5;column<=13;column++)sheet.getCell(index,column).alignment={horizontal:'right',vertical:'top',wrapText:false};sheet.getCell(index,14).alignment={horizontal:'center',vertical:'top',wrapText:true};const lines=Math.max(1,...[2,3,14,15].map(column=>String(sheet.getCell(index,column).value??'').split('\n').length));sheet.getRow(index).height=Math.min(60,Math.max(20,lines*15))}
  sheet.autoFilter={from:{row:15,column:1},to:{row:Math.max(15,row-1),column:15}};sheet.getRow(1).height=26;sheet.getRow(2).height=24;
