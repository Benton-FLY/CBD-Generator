  ...
  try{
   ...
   for(const sheet of sheets){
    ...
    for(let r=header.row+1;r<sheet.rows.length;r++){
     ...
     const labeled=row.map((value,column)=>({key:summaryKey(value),raw:label(value),column})).find(item=>item.key&&(!EXCLUDED_FOB_LABELS.has(item.raw)||item.key==='materialToFobRatio'));
