 ...
 for(const file of files){
  try{
   ...
   for(const sheet of sheets){
    ...
    let group='',order=0;
    const materials:CbdMaterialRow[]=[],summary:CbdSummary={},groupOrder:string[]=[],groupTotals:Record<string,number>={},duplicates=new Map<string,number>();
    for(let r=header.row+1;r<sheet.rows.length;r++){
     const row=sheet.rows[r]||[],material=String(row[header.map.material]??'').trim(),groupCell=String(row[header.map.group]??'').trim(),extended=num(row[header.map.extended]),joined=normalizeText(row.join(' ')),sk=summaryKey(joined);
     if(sk){summary[sk]=extended??num(row.find(v=>num(v)!==undefined));continue}
    ...
    const id=`${side}::${fileId}::${sheet.name}`;
