import {MATERIAL_MATCHER_VERSION,matchMaterials} from './matcher';
...
export async function loadComparison(){const db=await open();try{const state=await done(db.transaction(STORE).objectStore(STORE).get(KEY)) as ComparisonState|undefined;return state?upgradeComparison(migrate(state)):undefined}finally{db.close()}}
