...
export interface MaterialMatchEvidence {score:number;method:'Exact'|'Code Match'|'Normalized'|'Token Match'|'Similar'|'Merged N:1'|'Needs Review';matchedCodes:string[];matchedTokens:string[];ignoredReference:string[];ignoredComparison:string[];groupAssessment:string;unitAssessment:string;reason:string}
...
export type MaterialMatchCluster={id:string;referenceRowIds:string[];currentRowId:string|null;relationType:'one-to-one'|'many-to-one';matchSource:'auto'|'manual';manualLocked?:boolean;finalGroup:string;status:MaterialStatus;confidence:number;evidence?:MaterialMatchEvidence};
