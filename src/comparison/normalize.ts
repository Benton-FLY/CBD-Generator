...
const productTypes=['HANG TAG','CARE LABEL','WOVEN LABEL','PRINT MATERIAL','WEBBING','BUCKLE','ZIPPER','ELASTIC','LABEL','STICKER','PATCH','FABRIC','THREAD','VELCRO','EYELET','SNAP','BUTTON','TAPE'];
...
export function extractMaterialAttributes(value:string):MaterialAttributes {
  ...
  const codes=unique([...(normalized.match(/(?<![#A-Z0-9])\b[A-Z]{2,}[ -]?\d{3,}[A-Z0-9-]*\b/g)||[]),...(normalized.match(/#[A-Z0-9]+(?:\s+[A-Z]\d+)?/g)||[]),...(normalized.match(/\b(?:JX|VT|ES|PB|VSC)-[A-Z0-9-]+\b/g)||[])].map(code=>code.replace(/\s+/g,' ')));
  const specs=unique(normalized.match(/\b\d+(?:\.\d+)?(?:MM|CM|D|T)\b/g)||[]),styles=unique(normalized.match(/\b[A-Z]+-\d+[A-Z0-9-]*\b/g)||[]);
  ...
  const foundProducts=present(productTypes).filter(type=>type!=='LABEL'||!normalized.includes('WOVEN LABEL')&&!normalized.includes('CARE LABEL')),foundPositions=present(positions),foundDemographics=present(demographics);
  const supplierTokens=unique(Array.from(normalized.matchAll(/\(([^()]*)\)/g)).flatMap(match=>{const content=match[1].trim();return /^(?:[A-Z]{1,12}|[A-Z]+\s+[A-Z]+)$/.test(content)&&! /\b(?:NYLON|POLY|POLYESTER|PLASTIC|METAL|COTTON|INNER|OUTER|LEFT|RIGHT|PU|PA|WRCO)\b/.test(content)&&!codes.some(code=>code.includes(content))&&!specs.some(spec=>content.includes(spec))?content.split(/\s+/):[]}));
  const seasonTokens=unique(Array.from(normalized.matchAll(/\bFLY RACING ((?:20)?\d{2})\b/g),match=>match[1]));
  const label=foundProducts.some(type=>/TAG|LABEL|STICKER/.test(type));
  const contextual:string[]=label?(normalized.match(/\b(?:PANT|JERSEY)\b/g)||[]):[];
  if(foundProducts.includes('WEBBING')&&/\bP$/.test(normalized))contextual.push('P');
  const explicitAux=unique([...contextual,...supplierTokens,...(label?seasonTokens:[]),...[...auxiliaryWords].filter(word=>new RegExp(`(?:^|\\s)${word}(?:$|\\s)`).test(normalized))]);
  const codeParts=new Set(codes.flatMap(code=>code.replace('#','').split(/\s+/)));
  const coreTokens=unique(normalized.replace(/[(),/+]/g,' ').split(/\s+/).filter(token=>token&&token!=='AND'&&!auxiliaryWords.has(token)&&!explicitAux.includes(token)&&!codeParts.has(token)));
